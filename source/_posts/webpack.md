...
1. Webpack 入门实战：核心概念和常用配置一次讲清楚
2. 前端工程化别只会跑命令：这篇带你搞懂 Webpack
3. 手把手梳理 Webpack：从打包原理到配置思路

## 摘要

这篇文章适合想补齐前端工程化基础的同学，围绕 Webpack 的概念、配置和使用场景做系统梳理，方便上手项目打包。
...
下面进入正文。建议大家把配置项和打包结果联系起来看，这样更容易理解 Webpack 的工作方式。

如果你项目里已经有 Webpack 配置，可以对照这里的概念看看每个配置项到底在解决什么问题。
...
你现在项目还在用 Webpack，还是已经切到 Vite？如果想看一次真实项目配置拆解，可以留言。
