...
1. Shopify 开发实战：Shopify 自定义 PDP 一篇讲清楚
2. 手把手梳理 Shopify：Shopify 自定义 PDP 的关键用法和避坑点
3. 做 Shopify 项目别只会改主题，这篇带你搞懂 Shopify 自定义 PDP

## 摘要

这篇文章适合 Shopify 主题和应用开发者，围绕「Shopify 自定义 PDP」梳理核心概念、配置路径和实战注意点，方便项目中快速对照。
...
这篇围绕「Shopify 自定义 PDP」把原始笔记整理成更适合阅读和复用的版本，适合做主题开发、应用开发或者接手 Shopify 项目时快速对照。
...
1. 在`templates/*`下从旧的文件复制或直接新建一个`product.{id}.json`，提交文件到store

2. 后台配置产品和template关联

![alt text](1.png)

3. 新增/修改一些用到section、snippet
...
你现在做 Shopify 更常遇到的是主题开发、Checkout 定制、应用接入，还是性能优化？如果你在「Shopify 自定义 PDP」上踩过坑，也可以把场景留言出来。
